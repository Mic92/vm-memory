...
pub fn process_load<T: Sized + Copy>(pid: Pid, addr: *const c_void) -> Result<T, Error> {
    ...
    let aligned = (addr as usize) & !(ALG - 1);
    let offset = (addr as usize) - aligned;
...
pub fn process_read_bytes(pid: Pid, buf: &mut [u8], addr: *const c_void) -> Result<usize, Error> {
    let len = buf.len();
    let mut local_iovec = vec![IoSliceMut::new(buf)];
    let remote_iovec = vec![RemoteIoVec {
        base: addr as usize,
        len,
    }];

    let f = process_vm_readv(pid, &mut local_iovec, remote_iovec.as_slice())
        .map_err(Error::Rw)?;
...
pub fn process_write_bytes(pid: Pid, addr: *mut c_void, val: &[u8]) -> Result<usize, Error> {
    let len = val.len();
    let local_iovec = vec![IoSlice::new(val)];
    let remote_iovec = vec![RemoteIoVec {
        base: addr as usize,
        len,
    }];
